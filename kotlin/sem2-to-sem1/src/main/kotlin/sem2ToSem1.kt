            ...
            return when (translated) {
                ...
                is IntegerLiteralExpression -> {
                    val literalType = if (typeHint is S2Type.NamedType) {
                        // TODO: Is this too dumb? We probably want to check that it's a numeric-ish type and not a string type,
                        // so we can avoid translating and the error message will be less confusing... We might also want to
                        // offer a way to opt out of this automatic translation
                        // TODO: I do kind of want a metadata-collecting step of "these types look numerical, these look stringy"
                        translate(typeHint)
                    } else {
                        UnvalidatedType.Integer()
                    }
                    RealExpression(Expression.Literal(literalType, translated.literal, expression.location), UnvalidatedType.Integer())
                }
            }
        ...
        return when (expression) {
            ...
            is S2Expression.IfThen -> {
                ...
                RealExpression(Expression.IfThen(
                        condition = translateFullExpression(expression.condition, S2Type.Boolean(), varTypes).expression,
                        thenBlock = thenBlock,
