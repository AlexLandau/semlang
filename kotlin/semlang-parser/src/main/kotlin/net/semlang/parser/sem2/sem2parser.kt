package net.semlang.parser.sem2
...
// Note: These methods effectively transpile into sem1.

fun parseStringAsSem2(text: String, documentUri: String): ParsingResult {
    val stream = ANTLRInputStream(text)
    return parseANTLRStreamInner(stream, documentUri)
}


sealed class Sem2ParsingResult {
    abstract fun assumeSuccess(): RawContext
    data class Success(val context: RawContext): Sem2ParsingResult() {
        override fun assumeSuccess(): RawContext {
            return context
        }
    }
    data class Failure(val errors: List<Issue>, val partialContext: RawContext): Sem2ParsingResult() {
        init {
            if (errors.isEmpty()) {
...
// TODO: See if we can pull out the scoping step entirely, or rewrite (has seen too many changes now)
    ...
    private fun parseAssignments(assignments: Sem2Parser.AssignmentsContext): List<AmbiguousAssignment> {
        return parseLinkedList(assignments,
                Sem2Parser.AssignmentsContext::assignment,
                Sem2Parser.AssignmentsContext::assignments,
                this::parseAssignment)
    }

    private fun parseAssignment(assignment: Sem2Parser.AssignmentContext): AmbiguousAssignment {
